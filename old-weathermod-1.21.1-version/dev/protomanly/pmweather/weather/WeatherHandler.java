      ...
      for (Storm storm : this.getStorms()) {
         if (!storm.visualOnly) {
            ...
            if (storm.stormType == 2) {
               ...
               float intensity = (float)Math.pow(Math.clamp(storm.windspeed / 65.0F, 0.0F, 1.0F), 0.85F);
               ...
               staticBands *= (float)Math.pow(Math.clamp(dist / (storm.maxWidth * 0.25F), 0.0, 1.0), 0.1F);
               staticBands *= 1.25F * (float)Math.pow(intensity, 0.75);
               if (staticBands < 0.0F) {
                  weak += Math.abs(staticBands);
               } else {
                  weak += Math.abs(staticBands) * (float)Math.pow(1.0 - Math.clamp(dist / (storm.maxWidth * 0.65F), 0.0, 1.0), 0.5);
               ...
               rotatingBands *= (float)Math.pow(Math.clamp(dist / (storm.maxWidth * 0.25F), 0.0, 1.0), 0.1F);
               ...
               localRain = Math.max(
                  (float)Math.pow(1.0 - Math.clamp(dist / (storm.maxWidth * efc), 0.0, 1.0), 0.5) * (Math.abs(eye * 0.1F) + 0.9F) * 1.35F * intensity,
                  localRain
               );
               localRain *= (float)Math.pow(1.0 - Math.clamp(dist / storm.maxWidth, 0.0, 1.0), 0.5);
               localRain *= Mth.lerp(
                  0.5F + Math.clamp((storm.windspeed - 65.0F) / 40.0F, 0.0F, 1.0F) * 0.5F,
                  1.0F,
                  (float)Math.pow(Math.clamp(dist / (storm.maxWidth * 0.1F), 0.0, 1.0), 2.0)
               );
               if (localRain > 0.6F) {
            ...
            if (storm.stormType == 0) {
               ...
               if (storm.stage >= 2) {
                  perc *= Mth.lerp(Math.clamp(smoothStage - 2.0F, 0.0F, 1.0F), 1.0F, storm.occlusion * 0.5F + 0.5F);
               }
               ...
               if (storm.stage >= 2) {
                  p *= Math.clamp((smoothStage - 2.0F) / 0.5F, 0.0F, 1.0F);
               }
   ...
   public void read() {
      ...
      for (String tagName : storms.getAllKeys()) {
         ...
         try {
            ...
            PMWeather.LOGGER.error(var9.getMessage(), var9);
         }
