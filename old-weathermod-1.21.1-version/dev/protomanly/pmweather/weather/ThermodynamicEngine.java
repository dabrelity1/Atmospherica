...
public class ThermodynamicEngine {
   ...
   public static ThermodynamicEngine.AtmosphericDataPoint samplePoint(
      ...
      if (ThermodynamicEngine.noise == null) {
         ...
         double timeFactorHeightAffected = Mth.lerp(Math.clamp(altitude / pblHeight, 0.0F, 1.0F), timeFactor, 1.0);
         ...
         stormCooling *= 1.0F - Math.clamp(advance / 12000.0F, 0.0F, 1.0F);
         ...
         cachedNoise = noise;
         float bumpH = elevation + Math.clamp(noise + 0.5F, 0.5F, 1.5F) * 1250.0F;
         noise = FBM(pos.multiply(1.0F / -xzScale, 0.0, 1.0F / xzScale).add(0.0, time / timeScale, 0.0), 2, 2.0F, 0.5F, 1.0F);
         ...
         noise = FBM(pos.multiply(1.0F / xzScale, 0.0, 1.0F / xzScale).add(0.0, time / timeScale, 0.0), 2, 2.0F, 0.5F, 1.0F);
         float inversionHeight = elevationSeaLevel + Mth.lerp(Math.clamp(noise, 0.0F, 1.0F), 12000.0F, 16000.0F);
