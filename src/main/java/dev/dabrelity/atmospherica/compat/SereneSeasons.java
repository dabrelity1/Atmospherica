...
public class SereneSeasons {
   ...
   public static float getBiomeTemperature(Level level, Holder<Biome> biome, BlockPos pos) {
      ...
      if (isInstalled() && method_sereneseasons_getBiomeTemperature != null) {
         try {
            ...
            Atmospherica.LOGGER.error(var7.getMessage(), var7);
         }
      }
   ...
   public static boolean isInstalled() {
      if (!hasCheckInstalled) {
         ...
         if (installed) {
            Atmospherica.LOGGER.info("Atmospherica Compatibility found Serene Seasons");
         } else {
            Atmospherica.LOGGER.info("Atmospherica Compatibility did not find Serene Seasons");
         }
      }
