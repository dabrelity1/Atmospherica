               ...
               for (Storm storm : weatherHandler.getStorms()) {
                  if (!storm.visualOnly) {
                     ...
                     if (storm.stormType == 2) {
                        ...
                        double multBase = 1.0 - Mth.clamp(distance / storm.maxWidth, 0.0, 1.0);
                        float mult = (float)(multBase * multBase * multBase);
                        if (distance < storm.maxWidth * 0.1F) {
                           double fracBase = 1.0 - Mth.clamp(distance / (storm.maxWidth * 0.1F), 0.0, 1.0);
                           mult += (float)(Math.sqrt(fracBase) * Math.sqrt(Math.sqrt(fracBase))) * 0.15F;
                        ...
                        double m25Base = 1.0 - Mth.clamp((distance - storm.maxWidth * 0.1F) / (storm.maxWidth * 0.1F), 0.0, 1.0);
                        mult *= 1.15F + (float)(m25Base * m25Base * Math.sqrt(m25Base)) * 0.35F;
                        ...
                        double lerpBase = Mth.clamp(storm.windspeed / 65.0F, 0.0F, 1.0F);
                        mult *= Mth.lerp((float)(lerpBase * lerpBase), 1.0F, eye);
                     ...
                     if (storm.stormType == 1) {
                        ...
                        double fwdClamp = Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
                        Vec2 off = Util.mulVec2(
                           fwd,
                           -((float)(fwdClamp * fwdClamp))
                        ...
                        double gnBase = 0.8F + gustNoise * 0.5F;
                        perc *= Mth.lerp(
                           Mth.clamp(behind / ((float)ServerConfig.stormSize * 3.0F), 0.0F, 1.0F), (float)(gnBase * Math.sqrt(gnBase)), 0.5F
                        );
