               ...
               for (Storm storm : weatherHandler.getStorms()) {
                  if (!storm.visualOnly) {
                     ...
                     if (storm.stormType == 2) {
                        ...
                        float distRatio = 1.0F - (float)Mth.clamp(distance / storm.maxWidth, 0.0, 1.0);
                        float mult = distRatio * distRatio * distRatio;
                        if (distance < storm.maxWidth * 0.1F) {
                           float distRatio2 = 1.0F - (float)Mth.clamp(distance / (storm.maxWidth * 0.1F), 0.0, 1.0);
                           mult += (float)(Math.sqrt(distRatio2) * Math.sqrt(Math.sqrt(distRatio2))) * 0.15F;
                        ...
                        mult += Mth.lerp(
                           ...
                           Math.abs(bands) * Math.abs(bands) * 0.5F * mult,
                           0.5F * mult
                        );
                        ...
                        float distRatio3 = 1.0F - (float)Mth.clamp((distance - storm.maxWidth * 0.1F) / (storm.maxWidth * 0.1F), 0.0, 1.0);
                        mult *= 1.15F + (distRatio3 * distRatio3 * (float)Math.sqrt(distRatio3)) * 0.35F;
                        ...
                        float windRatio = Mth.clamp(storm.windspeed / 65.0F, 0.0F, 1.0F);
                        mult *= Mth.lerp(windRatio * windRatio, 1.0F, eye);
                     ...
                     if (storm.stormType == 1) {
                        ...
                        float clampDist = (float)Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
