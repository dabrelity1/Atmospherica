               ...
               for (Storm storm : weatherHandler.getStorms()) {
                  if (!storm.visualOnly) {
                     ...
                     if (storm.stormType == 2) {
                        ...
                        // Optimization: Avoided Math.pow(x, 3.0) and hoisted intermediate to avoid repetitive native calls
                        float multBase = 1.0F - (float)Mth.clamp(distance / storm.maxWidth, 0.0, 1.0);
                        ...
                        // Optimization: Expanded Math.pow(Math.abs(bands), 2.0)
                        float absBands = Math.abs(bands);
                        mult += Mth.lerp(
                        ...
                        // Optimization: Pre-calculated inner argument and replaced Math.pow(x, 2.0)
                        float clampedWind = (float)Mth.clamp(storm.windspeed / 65.0F, 0.0F, 1.0F);
                        mult *= Mth.lerp(clampedWind * clampedWind, 1.0F, eye);
                     ...
                     if (storm.stormType == 1) {
                        ...
                        // Optimization: Expanded Math.pow(..., 2.0) to x * x and lifted the argument computation
                        float clampedDist = (float)Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
                        Vec2 off = Util.mulVec2(
                           fwd,
                           -(clampedDist * clampedDist) * ((float)ServerConfig.stormSize * 1.5F)
                        );
                        le = le.add(off);
                        ri = ri.add(off);
