      ...
      if (level == null) {
         ...
         if (!ignoreStorms && !ignoreTornadoes) {
            for (Storm tornadicStorm : tornadicStorms) {
               ...
               Vec3 inwardx = new Vec3(-relativePosx.x, 0.0, -relativePosx.z).normalize();
               Vec3 rotationalx = new Vec3(relativePosx.z, 0.0, -relativePosx.x).normalize();
               // Bolt Performance Optimization: Replace distanceTo with distanceToSqr to eliminate Math.sqrt
               // in high frequency storm-checking loops.
               double distSqr = position.distanceToSqr(tornadicStorm.position);
               double maxDist = tornadicStorm.width * 2.0F;
