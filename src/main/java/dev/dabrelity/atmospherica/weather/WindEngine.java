               ...
               for (Storm storm : weatherHandler.getStorms()) {
                  if (!storm.visualOnly) {
                     ...
                     if (storm.stormType == 2) {
                        ...
                        double distClamp = 1.0 - Mth.clamp(distance / storm.maxWidth, 0.0, 1.0);
                        float mult = (float)(distClamp * distClamp * distClamp);
                        ...
                        float windClamp = Mth.clamp(storm.windspeed / 65.0F, 0.0F, 1.0F);
                        mult *= Mth.lerp((windClamp * windClamp), 1.0F, eye);
                     ...
                     if (storm.stormType == 1) {
                        ...
                        double distClamp2 = Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
                        Vec2 off = Util.mulVec2(
                           fwd,
                           -((float)(distClamp2 * distClamp2))
