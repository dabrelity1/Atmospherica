...
import dev.dabrelity.atmospherica.util.ShaderCompatibleNoise;
...
public class Clouds {
   public static float getCloudDensity(WeatherHandler weatherHandler, Vector2f location, float partialTicks) {
      ...
      float v = Mth.clamp(densityNoise - (1.0F - (float)ServerConfig.overcastPercent), 0.0F, 1.0F);
      ...
      for (Storm storm : weatherHandler.getStorms()) {
         ...
         if (storm.stormType == 2) {
            c *= Mth.lerp(
               Mth.clamp((storm.windspeed - 65) / 60.0F, 0.0F, 1.0F), 1.0F, (float)Math.pow(Mth.clamp(dist / (storm.maxWidth * 0.1), 0.0, 1.0), 2.0)
            );
         }

         if (storm.stormType == 0) {
            c *= Mth.lerp(Mth.clamp(smoothStage, 0.0F, 1.0F), 1.0F, (float)Math.pow(Mth.clamp(dist / (stormSize * 4.0), 0.0, 1.0), 1.5));
         }
      }
