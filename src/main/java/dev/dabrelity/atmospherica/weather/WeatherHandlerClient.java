...
public class WeatherHandlerClient extends WeatherHandler {
   ...
   public float getHail() {
      ...
      if (player == null) {
         ...
         for (Storm storm : this.getStorms()) {
            if (!storm.visualOnly) {
               double dist = dev.dabrelity.atmospherica.util.Util.distance2D(player.position(), new Vec3(storm.position.x + 2000.0, player.position().y, storm.position.z - 900.0));
               if (!(dist > ServerConfig.stormSize * 4.0)) {
                  double perc = 0.0;
                  if (storm.stormType == 0) {
                     perc = 1.0 - Mth.clamp(dist / (ServerConfig.stormSize * 6.0), 0.0, 1.0);
                     if (storm.stage == 2) {
   ...
   public void strike(Vec3 pos) {
      ...
      if (player != null) {
         ...
         if (dist > 256.0) {
            this.getWorld()
               .playLocalSound(
                  pos.x, pos.y, pos.z, ModSounds.THUNDER_FAR.get(), SoundSource.WEATHER, 5000.0F, Atmospherica.RANDOM.nextFloat(0.8F, 1.0F), true
               );
         } else {
            this.getWorld()
               .playLocalSound(
                  pos.x, pos.y, pos.z, ModSounds.THUNDER_NEAR.get(), SoundSource.WEATHER, 5000.0F, Atmospherica.RANDOM.nextFloat(0.8F, 1.0F), true
               );
         }
      }
   }
   ...
   public void tick() {
      ...
      while (iterator.hasNext()) {
         Lightning lightning = (Lightning)iterator.next();
   ...
   public void nbtSyncFromServer(CompoundTag compoundTag) {
      ...
      if (command.equals("syncStormNew")) {
         ...
         Atmospherica.LOGGER.debug("syncStormNew, ID: {}", ID);
         ...
         if (Atmospherica.RANDOM.nextFloat() > ClientConfig.debrisParticleDensity) {
            return;
         }
         ...
         if (storm != null) {
            ParticleCube debris = new ParticleCube(
               ...
               pos.x + (Atmospherica.RANDOM.nextFloat() - Atmospherica.RANDOM.nextFloat()) * 3.0F,
               pos.y + (Atmospherica.RANDOM.nextFloat() - Atmospherica.RANDOM.nextFloat()) * 3.0F,
               pos.z + (Atmospherica.RANDOM.nextFloat() - Atmospherica.RANDOM.nextFloat()) * 3.0F,
               0.0,
               0.0,
               0.0,
               state
