...
public abstract class WeatherHandler implements IWorldData {
   ...
   public float getPrecipitation(Vec3 pos) {
      ...
      for (Storm storm : this.getStorms()) {
         if (!storm.visualOnly) {
            double dist = dev.dabrelity.atmospherica.util.Util.distance2D(pos, storm.position.x, storm.position.z);
            double perc = 0.0;
            ...
            if (storm.stormType == 0) {
               double coreDist = dev.dabrelity.atmospherica.util.Util.distance2D(pos, storm.position.x + 2000.0, storm.position.z - 900.0);
