      ...
      if (ThermodynamicEngine.noise == null) {
         ...
         for (Storm storm : weatherHandler.getStorms()) {
            if (storm.stormType == 1) {
               double tempPosX = posX - storm.position.x;
               double tempPosZ = posZ - storm.position.z;
               double distance = Math.sqrt(tempPosX * tempPosX + tempPosZ * tempPosZ);
               ...
               double tempDist = Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
               Vec2 off = Util.mulVec2(
                  fwd, -((float)(tempDist * tempDist)) * ((float)ServerConfig.stormSize * 1.5F)
               );
               le = le.add(off);
               ri = ri.add(off);
