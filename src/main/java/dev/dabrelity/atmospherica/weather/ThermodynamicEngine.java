      ...
      if (ThermodynamicEngine.noise == null) {
         ...
         for (Storm storm : weatherHandler.getStorms()) {
            if (storm.stormType == 1) {
               ...
               double distClamp = Mth.clamp(distance / ((float)ServerConfig.stormSize * 5.0F), 0.0, 1.0);
               Vec2 off = Util.mulVec2(
                  fwd, -((float)(distClamp * distClamp)) * ((float)ServerConfig.stormSize * 1.5F)
               );
               le = le.add(off);
               ri = ri.add(off);
         ...
         float var81;
         if (humidity > 0.5F) {
            var81 = (float)Math.sqrt(Math.sqrt(2.0F * (humidity - 0.5F))) + 0.5F;
         } else {
            float h2 = 2.0F * humidity;
            var81 = (float)(h2 * h2 * h2 * h2) * 0.5F;
         }
         ...
         double pyFactor = posY / 16000.0;
         dewMin += (float)(pyFactor * pyFactor) * 40.0F * (1.0F - humidity);
