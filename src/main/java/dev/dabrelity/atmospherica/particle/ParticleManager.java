               ...
               for (int i = 0; i <= maxRenderOrder; i++) {
                  ...
                  if (particlesSorted != null && !particlesSorted.isEmpty()) {
                     ...
                     for (int j = 0; j < size; ++j) {
                        Particle particle = particlesSorted.get(j);
                        double distSqD = cameraPos.distanceToSqr(particle.getPos());
                        float distSq = (float) distSqD;
                        this.sortArray[j] = ((long)Float.floatToRawIntBits(distSq) << 32) | ((long)j & 4294967295L);
                     }
