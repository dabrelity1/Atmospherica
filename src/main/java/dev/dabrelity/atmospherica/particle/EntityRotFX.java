...
public class EntityRotFX extends TextureSheetParticle {
   ...
   public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
      ...
      Vector3f vec0 = new Vector3f(-1.0F, -1.0F, 0.0F);
      vec0.rotate(quaternion).mul(scale).add(f, f1, f2);

      Vector3f vec1 = new Vector3f(-1.0F, 1.0F, 0.0F);
      vec1.rotate(quaternion).mul(scale).add(f, f1, f2);

      Vector3f vec2 = new Vector3f(1.0F, 1.0F, 0.0F);
      vec2.rotate(quaternion).mul(scale).add(f, f1, f2);

      Vector3f vec3 = new Vector3f(1.0F, -1.0F, 0.0F);
      vec3.rotate(quaternion).mul(scale).add(f, f1, f2);
