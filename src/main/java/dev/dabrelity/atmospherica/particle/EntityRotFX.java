...
public class EntityRotFX extends TextureSheetParticle {
   ...
   public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
      ...
      if (this.facePlayer || this.rotationPitch == 0.0F && this.rotationYaw == 0.0F) {
         // Bolt: Use copy constructor instead of clone() to avoid try-catch overhead
      ...
      // Bolt: Reuse single mutable Vector3f to avoid allocating array + 4 objects per particle
      Vector3f vec = new Vector3f();

      // Vertex 0
      vec.set(-1.0F, -1.0F, 0.0F);
      vec.rotate(quaternion).mul(scale).add(f, f1, f2);
      buffer.vertex(vec.x, vec.y, vec.z).uv(u1, v1).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();

      // Vertex 1
      vec.set(-1.0F, 1.0F, 0.0F);
      vec.rotate(quaternion).mul(scale).add(f, f1, f2);
      buffer.vertex(vec.x, vec.y, vec.z).uv(u1, v0).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();

      // Vertex 2
      vec.set(1.0F, 1.0F, 0.0F);
      vec.rotate(quaternion).mul(scale).add(f, f1, f2);
      buffer.vertex(vec.x, vec.y, vec.z).uv(u0, v0).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();

      // Vertex 3
      vec.set(1.0F, -1.0F, 0.0F);
      vec.rotate(quaternion).mul(scale).add(f, f1, f2);
      buffer.vertex(vec.x, vec.y, vec.z).uv(u0, v1).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();
   }
