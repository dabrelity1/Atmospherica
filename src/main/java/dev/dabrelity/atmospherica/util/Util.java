...
public class Util {
   public static final int MAX_RAIN_DROPS = 2000;
   ...
   public static boolean canWindAffect(Vec3 pos, Level level) {
      BlockHitResult upRay = level.clip(
         new ClipContext(
            pos.add(0.0, 0.55, 0.0), pos.add(0.0, 128.0, 0.0), net.minecraft.world.level.ClipContext.Block.COLLIDER, Fluid.NONE, null
         )
      );
      BlockHitResult pxRay = level.clip(
         new ClipContext(
            pos.add(1.0, 0.55, 0.0), pos.add(64.0, 128.0, 0.0), net.minecraft.world.level.ClipContext.Block.COLLIDER, Fluid.NONE, null
         )
      );
      ...
      BlockHitResult pzRay = level.clip(
         new ClipContext(
            pos.add(0.0, 0.55, 1.0), pos.add(0.0, 128.0, 64.0), net.minecraft.world.level.ClipContext.Block.COLLIDER, Fluid.NONE, null
         )
      );
   ...
   public static Vec3 getValidTropicalSystemSpawn(WeatherHandler weatherHandler, Vec3 origin, float area) {
      for (int i = 0; i < 35; i++) {
         Vec3 check = origin.add(Atmospherica.RANDOM.nextFloat(-area, area), 0.0, Atmospherica.RANDOM.nextFloat(-area, area));
   ...
   static {
      ...
      for (int i = 0; i < MAX_RAIN_DROPS; i++) {
         RAIN_POSITIONS[i] = new Vec3(
            Atmospherica.RANDOM.nextFloat() * range - range / 2.0F,
            Atmospherica.RANDOM.nextFloat() * range - range / 2.0F,
            Atmospherica.RANDOM.nextFloat() * range - range / 2.0F
         );
      }
