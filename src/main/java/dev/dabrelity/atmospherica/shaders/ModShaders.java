    ...
    public static void renderShaders(
        ...
        if (
            ...
            if (clouds instanceof PostChainMixin mixin) {
                ...
                double sunIntensityVal = (Math.cos(sunAngle) + 1.0) / 2.0;
                setUniformFloat(effect, "lightIntensity", 
                    (float) (sunIntensityVal * sunIntensityVal * sunIntensityVal));
                ...
                Vec3 samplePos = new Vec3(camera.getPosition().x, ServerConfig.layer0Height, camera.getPosition().z);
